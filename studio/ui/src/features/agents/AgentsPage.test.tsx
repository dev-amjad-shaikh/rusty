...
function renderPage() {
  ...
  const router = createRouter({ routeTree: root.addChildren([agents, builder, workspace, prompts]), history: createMemoryHistory({ initialEntries: ["/agents"] }) });
...
beforeEach(() => { clearAgentBuilderMemory(); useConnectionStore.setState({ connection: null, info: null, workspaceStatus: "unavailable", discoveryAttempt: 0, discoveryError: "", suggestedOrigin: "", dialogOpen: false }); useAgentMutationStore.setState({ uncertainByConnection: {} }); });
...
describe("Agents", () => {
  ...
  it("freezes the submitted definition and blocks departure until creation settles", async () => {
    ...
    await completeBuilder();
    await userEvent.click(screen.getByRole("button", { name: "Create agent" }));
    expect(screen.getByLabelText("Maximum steps")).toBeDisabled();
    void router.navigate({ to: "/agents/prompts" });
  ...
  it("records a late exact create in its originating workspace without restoring a committed draft", async () => {
    ...
    expect(await screen.findByRole("status")).toHaveTextContent("Research analyst was created in this workspace.");
    await userEvent.click(screen.getByRole("tab", { name: /Purpose/ }));
    expect(screen.getByLabelText("Name")).toHaveValue("");
    expect(screen.getByRole("button", { name: "Agent created" })).toBeDisabled();
  });
