...
describe("exception-led Operations", () => {
  ...
  it("closes selected evidence when connection ownership changes", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "1 item" })).toHaveFocus());
  });
  ...
  it("does not steal focus when refreshed evidence updates the selected item", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn().mockImplementation((input: string) => {
      const url = new URL(input);
      if (url.pathname === "/tasks" && url.search === "?status=dead") return Promise.resolve(response([{ task_id: "task-a", kind: "publish", pool: "default", status: "dead", last_error: refreshed ? "new evidence" : "old evidence", next_attempt_at: null, run_id: null, thread_id: null, updated_at: refreshed ? "2026-08-11T00:01:00Z" : "2026-08-11T00:00:00Z" }]));
      if (url.pathname === "/artifacts/journal") return Promise.resolve(response(emptyArtifactJournal));
      return Promise.resolve(response([]));
    }));
    renderPage();
  ...
  it("moves focus to a stable heading when an owned artifact inspector is dismissed", async () => {
    vi.stubGlobal("fetch", vi.fn().mockImplementation((input: string) => {
      const url = new URL(input);
      if (url.pathname === "/artifacts/journal") return Promise.resolve(response(unavailableArtifactJournal));
      if (url.pathname === `/artifacts/${unavailableArtifactId}`) return Promise.resolve(response(unavailableArtifact));
      if (url.pathname === `/artifacts/${unavailableArtifactId}/preview`) return Promise.resolve(response({ artifact_id: unavailableArtifactId, preview: { kind: "empty", reason: "Stored bytes unavailable" } }));
      return Promise.resolve(response([]));
    }));
    renderPage();
    await userEvent.click(await screen.findByRole("button", { name: "Review" }));
    ...
    await waitFor(() => expect(["attention-heading", "operations-heading"]).toContain(document.activeElement?.id));
  });
