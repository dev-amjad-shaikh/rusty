...
import { getPromptCandidate, getPromptHistory, listPromptArtifacts, savePromptVersion, StudioApiError, type PromptArtifact } from "../../lib/api/client";
...
export function PromptStudio() {
  ...
  const sourceRuns = useMemo(() => [...comparisons].reverse(), [comparisons]);
  ...
  const key = ["prompts"];
  const catalog = useQuery({ queryKey: key, queryFn: () => listPromptArtifacts() });
  useEffect(() => {
        ...
        try {
          ...
          if (candidateId) {
            ...
            if (candidate.candidate.content.name === name && candidate.candidate.content.prompt === draft && runs.includes(sourceRun)) {
              return { receipt: { candidateId, created: false, committed: true } };
            }
          }
        } catch { /* lock below */ }
        mutationState.markUncertain("Rusty may have saved part or all of this prompt version. Check its history before allowing another save.");
