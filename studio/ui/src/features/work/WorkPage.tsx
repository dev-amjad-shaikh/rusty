...
function previewEvents(evidence: RunEvidence | null) { if (!evidence?.events.length) return [{ label: "Agent starts", detail: "Waiting for an objective", done: false }, { label: "Model and tools", detail: "Appears as work happens", done: false }, { label: "Result and evidence", detail: "Captured at completion", done: false }]; const latest = evidence.events.slice(-3); return latest.map((event) => ({ label: traceEventTitle(event), detail: event.node_id ?? `Sequence ${event.seq}`, done: event.status === "ok" })); }
...
export function traceEventTitle(event: RunEvent) {
  if (event.kind === "tool_call") {
    const payload = event.input && typeof event.input === "object" && !Array.isArray(event.input)
      ? event.input as Record<string, unknown>
      : null;
    const value = payload?.kind === "inline" && payload.value && typeof payload.value === "object" && !Array.isArray(payload.value)
      ? payload.value as Record<string, unknown>
      : payload;
    const tool = typeof value?.tool === "string" && /^[A-Za-z0-9._:-]{1,128}$/.test(value.tool) ? value.tool : "";
    if (tool) return `${humanizeTraceIdentifier(tool)} · tool call`;
  }
  return event.node_id ? `${event.node_id} · ${event.kind.replaceAll("_", " ")}` : event.kind.replaceAll("_", " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
}
function humanizeTraceIdentifier(value: string) { return value.replaceAll("_", " ").replaceAll("-", " ").replace(/\b\w/g, (letter) => letter.toUpperCase()); }
