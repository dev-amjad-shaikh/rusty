...
function previewEvents(evidence: RunEvidence | null) { if (!evidence?.events.length) return [{ label: "Agent starts", detail: "Waiting for an objective", done: false }, { label: "Model and tools", detail: "Appears as work happens", done: false }, { label: "Result and evidence", detail: "Captured at completion", done: false }]; const latest = evidence.events.slice(-3); return latest.map((event) => ({ label: eventTitle(event), detail: event.node_id ?? event.kind.replaceAll("_", " "), done: event.status === "ok" })); }
