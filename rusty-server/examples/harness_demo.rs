//! Harness demo: three ReAct agents that walk the harness surfaces end to
//! end — connector service packs over stateful fixtures, per-run tool
//! allowlists, the composer lane, and an experiment evaluator — served on
//! `127.0.0.1:8110`.
//!
...
fn build_composer_graph() -> Result<(Graph, StateSpec, ToolRegistry)> {
    ...
    tools.register(PublishComposedSkillTool::new(session, registry));
...
async fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let (composer, composer_spec, composer_tools) = build_composer_graph()?;
    ...
    println!("  Graphs: calendar_manager, servicenow_operator, composer_studio");
