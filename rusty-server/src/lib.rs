...
//! | `POST /connections` | R0.11 (wave 3): register a connection `{provider, subject?, scopes, token}` → `201 {connection}`; the token material is envelope-encrypted before it touches the store and the registration journaled — the bytes never travel beyond the broker. Wave 4: `{provider, subject?, scopes, authorization_code}` exchanges the code through the deployment's OAuth provider (`ServerConfig::with_oauth_provider`; `409` when none is configured) and seals the grant. `password_grant` (provider `oauth2_password`) exchanges the resource-owner credentials through the same provider and seals both the minted tokens and the grant inputs, so the refresh lifecycle re-mints without a human — the three credential paths are mutually exclusive (`422`) |
...
//! | `POST /connections/{id}/consent` | R0.11 (wave 3): record a consent act `{scopes?, token?}` (one required, `422` otherwise) → `200 {connection, journaled}`; a scope-set change journals `connection_consented`, material-only `connection_refreshed`; re-recording the same fact converges without a second event. Re-activates a `needs_reauth` connection — this is the re-auth path. Wave 4: `authorization_code` is another material path — exchanged through the OAuth provider and sealed; `password_grant` re-exchanges an `oauth2_password` connection's credentials the same way. The material paths are mutually exclusive |
