...
| EP-01-S08 | Event-schema conformance suite and closed-enum versioning | P0 | ◐ | `event_schema_conformance.rs` (16 tests, 8 golden files) landed on `feat/event-schema-conformance` (`ae85eb4`): exhaustive round-trip for all 62 `RunEventKind` variants, unknown-tag rejection, golden-file schema-drift detection, closed-enum invariants on empty + executed journals; `SurfaceOp` boundary tests and `schemars` integration blocked by missing types/dependency |
