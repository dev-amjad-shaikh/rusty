...
| EP-17 Self-Improvement Measurement (survey fast-follows) | M2–M4 | 6 | 0 | 0 | 6 | ░░░░░░ 0% |
...
| Story | Title | P | Status | Evidence / what's open |
|---|---|---|---|---|
| EP-16-S01 | Phase 0 foundations: generated types package, REST/WS clients, tokens + controls, app shell | P0 | ○ | Generated wire types from Rust JSON Schema (no hand-declared API types); REST client with idempotency keys, cursors, RFC 9457 problem JSON; WS client with snapshot-on-connect, seq gap → re-snapshot, typed connection state; design tokens + core controls; app shell with sidebar groups (Build/Test/Operate), main card, connection banner, role/scope gating. Done when: shell renders with all nav groups gated by fixture scopes; banner shows on injected gap (build plan Phase 0; R-X1, R-X2) |
| EP-16-S02 | AgentDraft + schema-generated AgentDraftForm with anchored live validation | P0 | ○ | One `AgentDraft` type shared by Guided/Compose/Chat/Import/Improve (R-A1); field added to schema appears in every entry path without per-path code; live validation ≤300 ms on pause-typing with the full rule set from `04-data-model.md`, violations anchored to controls and spec files (R-A3); drafts autosave + Drafts screen with Resume/Discard (R-A9) |
| EP-16-S03 | Guided authoring: template gallery, slot cards, autonomy step | P1 | ○ | Templates = published blueprints flagged as templates, never instantiable; slot cards derived from the template document; Start blank renders the full form; unfilled slots surface as violations; autonomy three-card step with read_only coherence note against mounted Write/Egress tools (R-A4; validation table rows autonomy × toolsets) |
| EP-16-S04 | Compose: spec-file editor with parse/regenerate and read-only assembled prompt | P1 | ○ | Markdown+frontmatter file model per `04-data-model.md` (agent.md frontmatter authoritative for identity/connectors/skills/gate/channel); edits re-parse into the draft; dependent files regenerate; editor with gutter, tinting, status bar (CodeMirror 6 in production); assembled-prompt.txt read-only with byte count; Validation + Resolved bottom strip with click-to-file anchoring (R-A5) |
| EP-16-S05 | Chat builder: carrier session, envelope protocol, catalog-filtered merge | P1 | ○ | Session id in URL (survives refresh); outbound envelope = text + full draft + catalogs; inbound `<agent_draft>` merged only after id filtering against catalogs; block stripped (closed and unclosed forms); streaming; "Draft updated" chip per patch; parse failure = no form change (R-A6). Done when: builder proposals never introduce an id outside the catalogs |
| EP-16-S06 | Import: Claude Code / Hermes / Letta .af / OpenClaw with all-or-nothing resolution | P1 | ○ | Scan → mapping table (resolved/bind/unresolved) → draft with new blueprint_id; secrets become SecretRef placeholders; unresolved refs block publish; id_mappings returned (R-A7). Done when: an import with one unresolvable ref lands as a draft that cannot publish |
| EP-16-S07 | Review: the convergence screen — diff, governance flags, eval gate, publish, export | P0 | ○ | All entries converge here; the only place Publish exists (R-A2); field-level diff vs published head grouped by section; GOVERNANCE badges on autonomy/approval-scope/trigger changes; assembled prompt preview with bytes + copy; eval gate status with run/re-run (Not run/Running/Passing/Stale/Failing); publish preconditions (0 violations ∧ gate Passing ∧ publish scope); export bundle with no-secret-values scan asserted; fleet-upgrade offer on publish-with-sessions (R-A8, R-O7 hook) |
| EP-16-S08 | Goal & measures, structured tool rules, triggers | P1 | ○ | Measures typed by source (outcome/connector/eval) and kind (target/gate/guardrail), entering the stable tier and driving Metrics (R-A10); tool rules structured (tool or all + text), compiled into prompt and guard, dangling tool = violation (R-A11); triggers cron (5-field, next-firing hint) and connector events (mounted connectors only), seed prompt, queue policy; a trigger satisfies the channel requirement (R-A12) |
| EP-16-S09 | Catalog add-flows: registry install, channel adapter picker, plugins list | P1 | ○ | Connectors from a signed registry index (allowlisted → install; otherwise request → Inbox approval — a non-allowlisted install never proceeds without an approval record); channels from an adapter picker; installed-plugins list with doctor status and new-manifest path; custom MCP servers register as tools, wrappable as connectors (R-A13) |
| EP-16-S10 | Playground: isolated session, event inspector, guard stages, inline approvals | P0 | ○ | Isolated session from draft or any published version; inspector lists every event in order; tool events expand to the five guard stages (pre_execute/guards/execute/post_execute/finalize) with outcome colors; approvals render from obligation data and are decided inline; Save as eval case (R-T1) |
| EP-16-S11 | Evals workbench: suites, base-vs-head diff, gate labeling | P1 | ○ | Suites with recorded-session cases; pass/fail per version; base-vs-head diff (newly passing / regressed / unchanged, regressed rows tinted); run against version or draft; open case in Playground; suites labeled where they gate (R-T2). Done when: a regression is visible as "regressed" between two versions and opens in the inspector |
| EP-16-S12 | Operate: Inbox lanes + Work board | P1 | ○ | Inbox lanes action_required (pings) / attention / info / decided with accountability chain chips; sticky approvals when allowed; double decision refused; "Nothing needs you" empty state (R-O1); Work board from StatusCategory with agent/human assignees, attempts with FailureReason and retry chain, in_review → done only by a human with tasks:done, rerun, session link (R-O2) |
| EP-16-S13 | Operate: Observe (logs, metrics, traces) + Learning (memory, skills ledger, gap ledger) | P1 | ○ | All derived from the log with live seq shown; every row replays into the inspector (R-O3); memory blocks with contents/usage/recall/consolidation and Consolidate-now; skills ledger with retention, states, entries, two-entry diff, rollback-as-new-Rollback-entry, promote via gate, restore; gap ledger with priority, origin incl. speculative, Hunting/Parked with next probe, triage with reason, evidence/resolution links (R-O4) |
| EP-16-S14 | Improve: per-agent plans from outcomes, regressions, and gaps | P2 | ○ | Findings cite evidence; changes selectable with kind badges and confidence; Accept → draft vN+1 → Review; per-agent auto-apply after gate (publish scope). Open design question (handoff `07`): whether Improve touches skills directly or only proposes drafts — design defaults to propose-draft (R-O5) |
| EP-16-S15 | Security center v2: egress preview, secret refs, autonomy, audit replay, administration | P1 | ○ | Egress rules with audit-mode preview against 7d egress before apply; secret-ref registry with wire probes (no values anywhere); autonomy overview sorted full → read_only; audit search with HMAC status → replay, server-side export with hash; administration cards (org, SSO/SCIM test-login before enforce, retention with legal hold, budgets with typed refusal) (R-O6) |
| EP-16-S16 | Cross-cutting: connection states, scope rendering, a11y, i18n, degraded copy | P0 | ○ | Reconnecting/offline banner with last-snapshot seq; fallback-rendered lists labeled, never fallback-as-fact (R-X1); capabilities absent not disabled, role change re-routes to nearest permitted screen (R-X2); WCAG 2.1 AA — keyboard operable, focus-visible, ARIA composites, 4.5:1 contrast, live regions, reflow 360–1920 px with no horizontal scroll (R-X3); i18n-ready strings, locale dates/costs (R-X4); every degraded/empty state has the specified copy from `03-screens.md` (R-X5) |

## EP-17 — Self-Improvement Measurement (survey fast-follows)

░░░░░░ 0% · 0 landed · 0 partial · 6 not started · milestone M2–M4
...
| EP-17-S02 | Regression packs as a promotion-gate input (R2) | P0 | ○ | Canonical suite of previously-passing skill evals re-run on every promotion decision; failure blocks with a typed refusal naming the regressed cases. Builds on the conformance registry (EP-12-S09) persistence/versioning — no new storage |
| EP-17-S03 | Held-out split enforcement in eval gates (R3) | P0 | ○ | A candidate tuned against suite S cannot promote on S alone; the gate requires a held-out suite or a suite version bump. Pairs with EP-12-S09's existing version-bump-invalidation mechanism |
| EP-17-S04 | Attribution fields on eval artifacts (R4) | P1 | ○ | Promotion evaluation records which scaffold components changed vs baseline (prompt tier hash, memory high-water mark, skill pack version, model stamp). The journal already carries every input — this is a rollup on the eval artifact, not new instrumentation |
...
Statuses move when code lands on `main` (or a named branch, called out in the evidence column). Regenerate judgments after each merged wave: a story flips to ✅ only when its acceptance criteria in the spec file pass, not when its module exists. `docs/backlog.md` remains the separate platform-maturity-review backlog; this file tracks the Source-Code spec plus two owner-directed additions: EP-16 (Rustynome Studio v2 design handoff, `docs/studio-v2-design-handoff/`, 2026-09-05) and EP-17 (self-improvement measurement fast-follows from `docs/self-improving-agents-survey-notes.md`, 2026-09-04).
