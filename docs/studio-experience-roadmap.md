...
- The configuration workshop covers the current persisted contract: graph, runtime step limit, catalog
  ...
  tool-contract, and singular model-settings artifacts are now selectable as an ordered assistant-level
  preset that survives portable manifests and immutable versions. **Prepare governed run** resolves that
  preset against the live catalog, creates a thread without running it, and hands it to the existing
  environment-bound planner for fresh one-run acknowledgement and exact journal proof. Credential handles
  and generic provider enforcement remain; output and budget requirements are portable declarations rather
...
remain available for rollback. Each lineage is capped at 256 versions, 64 KiB per immutable body, and 1 MiB
total; persisted content addresses and active-pointer consistency are revalidated on load. Copies and
applied manifests now carry a live, source-bound change review across identity, enforced runtime fields,
portable intent, descriptive metadata, and preserved advanced values. It compares structural values while
...
access/scopes, an approval boundary, an output contract, exact run-budget ceilings, and an ordered governed
runtime preset without raw JSON. Rusty stores and round-trips that portable intent. Studio can prepare a
thread from the preset, but it does not run until the current catalog is reviewed and freshly acknowledged;
the registered graph still chooses providers outside this admission path. Archive and restore
...
typed capability discovery and governed runtime binding are delivered; credential handles and generic
provider enforcement remain.
...
state, forbidden-tool, cost, and latency checks. The download is a portable one-case dataset asset, not
a durable catalog entry, experiment result, or release decision. A connection-bound Dataset Workbench
can then combine sealed cases and imported format-v1 JSONL into one exact, bounded page-memory ledger.
It preserves ordering, deduplicates canonically identical cases, rejects identity or case conflicts atomically, exposes
...
repetition, assertion, and judge evidence. Aggregate drift remains visible rather than becoming a false
pass; future serde-compatible fields are disclosed but not interpreted. The Explorer is read-only and
does not claim execution, external-provider attestation, a gate decision, or promotion. Durable report
discovery, version attribution, experiment execution/progress, durable statistical-report storage, and
enforced gate decisions still need platform APIs. The Matched Regression Lab closes the local paired
pass-rate comparison gap: two reconciled reports must share the exact dataset, repetition contract, and
...
| 3 | Configuration workshop for instructions, behavior, limits, and advanced manifest | Partial — current runtime contract, manifest portability, visual model/tool/memory/approval/output/budget intent, typed extension discovery, portable assistant-level governed presets, and thread-scoped acknowledgement with journal proof delivered; credentials and generic provider enforcement remain | Replaces the current create-only form and raw configuration gap |
