...
| **R0.2 — Persistence** | Durability & streaming + server Phase A | Postgres checkpointer, token streaming (`messages` mode), live example; axum server: threads, runs, SSE, auth | `rusty-agent-runtime` v0.2.0, `rusty-agent-server` v0.1.0 | ✅ Implemented | 2026-08-05 |
| **R0.3 — Interop** | interop & distribution | MCP client, remote nodes + worker SDK, server API completion, executor tracing | `rusty-agent-runtime` v0.3.0, `rusty-agent-server` v0.2.0, `rusty-worker` v0.1.0 | ✅ Implemented | 2026-08-05 |
| **R0.4 — Time Travel** | production hardening | WASM nodes, time-travel core + server API, Postgres server store, OpenTelemetry export, Studio UI, permissive CORS | `rusty-agent-runtime` v0.4.0, `rusty-agent-server` v0.3.0, `rusty-otel` v0.1.0 | ✅ Implemented | 2026-08-05 |
| v0.5 (pre-1.0) | SDKs & tenancy | Python SDK (stdlib-only), TypeScript SDK (zero-dep ESM), multi-tenant auth with full isolation, live-LLM validation + calculator fix | `rusty-agent-server` v0.4.0, `sdks/*` v0.1.0 | ✅ Implemented | 2026-08-05 |
| **R0.5 — Flight Recorder** | evidence | Canonical contracts (RunEvent, DecisionEvent, Effect, checkpoint header with `format_version`), effect journal, determinism seams, exact/live/hybrid replay, fork + branch diff, portable replay fixtures | `rusty-agent-runtime` v0.5.0, `rusty-agent-server` v0.5.0 | ✅ Implemented | 2026-08-07 |
| **R0.6 — Durable Work** | effectively-once activities | Task queue (file + Postgres), leases + heartbeats, retry taxonomy with backoff + DLQ, cancellation propagation, idempotency keys, transactional outbox, effect receipts, worker draining, pools/quotas/version pinning, crash-recovery release proof | `rusty-agent-runtime` v0.6.0, `rusty-agent-server` v0.6.0, `rusty-worker` v0.3.0 | ✅ Implemented | 2026-08-08 |
| **R0.7 — Agent Fabric** | durable agent teams + state scaling | Durable agent identities, versioned capability manifests, typed mailboxes, supervision, task/artifact contracts; **effect kernel v2** (marker-trait enforcement, approval boundary for irreversible effects); **versioned run manifest** (prompts, tool schemas, model config pinned per run); coordination patterns + TeamTrace; copy-on-write state, delta checkpoints, content-addressed artifacts | `rusty-agent-runtime` v0.7.0, `rusty-agent-server` v0.7.0, `rusty-worker` v0.3.1 | ✅ Implemented | 2026-08-08 |
...
| **R0.10 — Adaptation** | executor policy learning | Headroom experiment (the gate — every priced family cleared the bar, published); **runtime digital twin** (fault schedules, schedule randomization, counterfactual forks landing hybrid replay, shadow policies with true propensities); learned retry/timeout policies through the policy plane with envelope validation, twin-gated promotion, drift detection, byte-exact revert-to-floor; server fail/claim paths wired, `GET /policy/drift` | `rusty-agent-runtime` v0.10.0, `rusty-agent-server` v0.10.0, `rusty-worker` v0.3.4 | ✅ Implemented | 2026-08-09 |
| **R0.11 — Extension Plane** | governed configuration + credentials | Prompt/configuration registry (named artifacts, environment-tagged pointers, read-computed diffs, CAS commits — an index over the R0.8 candidate pipeline); admission-time resolution with journaled `ConfigResolved` events and the receipt→pin→candidate→author audit walk; **credential/connection broker** (envelope-encrypted storage, opaque scoped handles, bytes only at the connector, journaled uses/denials); OAuth lifecycle + health surface; middleware compositions promoted per environment with an additive manifest pin | `rusty-agent-runtime` v0.11.0, `rusty-agent-server` v0.11.0, `rusty-worker` v0.3.5 | ✅ Implemented | 2026-08-10 |
| **R0.12 — Operations Plane** | artifacts + deployment control | Artifact plane (content-addressed artifacts with lineage, permissions, previews, versions, retention — extends the R0.7 artifact contracts); deployment control plane (immutable revisions, dev/staging/prod environments, canary + shadow deployments wired to eval release gates) | `rusty-agent-runtime` v0.12.0, `rusty-agent-server` v0.12.0, `rusty-worker` v0.3.6 | ✅ Implemented | 2026-08-11 |
...
### R0.2 — Persistence · durability & streaming + server Phase A — `rusty-agent-runtime` v0.2.0, `rusty-agent-server` v0.1.0 (2026-08-05)

Core gained the `sqlx`-backed `PostgresCheckpointer` (`postgres` feature), real token streaming (`ChatModel::chat_stream` → `GraphEvent::Token`, the LangGraph `messages` stream mode), and a live-agent example against any OpenAI-compatible endpoint. The new `rusty-agent-server` crate implemented Phase A of the Agent-Protocol surface: threads, background/blocking/SSE runs, checkpoint history, per-thread run queue, API-key auth — 10 integration tests green. Details: [CHANGELOG 2026-08-05](../CHANGELOG.md), [server design doc](rusty-server-design.md), [server quickstart](server-quickstart.md).

### R0.3 — Interop — `rusty-agent-runtime` v0.3.0, `rusty-agent-server` v0.2.0, `rusty-worker` v0.1.0 (2026-08-05)
...
- **Server API completion** (`rusty-agent-server` v0.2) — fills out the Agent-Protocol surface from the [design doc](rusty-server-design.md): `GET /runs/{id}`, assistants, crons, and the KV store — 20 integration tests green.
...
### R0.4 — Time Travel · production hardening — `rusty-agent-runtime` v0.4.0, `rusty-agent-server` v0.3.0, `rusty-otel` v0.1.0 (2026-08-05)
...
- **Postgres server store** (`rusty-agent-server`, feature `postgres`) — `ServerConfig::with_postgres(url)` moves run checkpoints *and* the assistants/crons/KV surface into Postgres (`server_*` tables, auto-migrated on first use; migrations serialize on a transaction-scoped advisory lock, so concurrent cold boots are safe).
...
### v0.5 — SDKs & tenancy (pre-1.0) — `rusty-agent-server` v0.4.0, `sdks/*` v0.1.0 (2026-08-05)
...
- **Multi-tenant auth** (`rusty-agent-server` v0.4.0) — `ServerConfig::with_tenant_key(tenant, key)` maps API keys to tenants; threads, runs, assistants, crons, and KV namespaces are fully isolated via internal `{tenant}/` id prefixing, cross-tenant access answers 404 (never 403), and open/dev mode stays byte-identical to before.
