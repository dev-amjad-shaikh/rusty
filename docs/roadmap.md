...
| **R0.9 — Capsules** | secure isolation + federation | WASM **Component Model** capsules (WIT worlds as language-neutral contracts), deny-by-default capability host, resource budgets (fuel/epoch interruption via Wasmtime), Cedar policy engine, policy overlays; **signed run receipts** over the hash-chained journal; MCP server bridge, A2A server/client with durable tasks/artifacts | `rusty-agent-runtime` v0.9.0 | 📋 Planned | — |
