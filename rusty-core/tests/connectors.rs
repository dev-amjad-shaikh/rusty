...
use rusty_agent_runtime::connector::{
    CheckOutcome, CheckRequest, CheckResponse, CheckStatus, ConnectorInstance, ConnectorManifest,
    ConnectorOperation, ConnectorTransport, HttpMethod, OperationAuth, OperationEffect,
    execute_check, extract_secrets, insert_masked_secrets, insert_opened_secrets,
    render_template, scan_placeholders, validate_config,
};
...
fn op(name: &str, method: HttpMethod, path: &str, effect: OperationEffect, params: Value) -> ConnectorOperation {
    ConnectorOperation {
        name: name.to_owned(),
...
fn masked_serving_marks_secrets_without_values() {
    ...
    sealed.insert("credentials.username".to_owned(), envelope("credentials.username"));
    let served = insert_masked_secrets(json!({"instance": "dev123"}), &sealed);
    assert_eq!(
        served,
        json!({"instance": "dev123", "credentials": {"username": {"rusty_secret": true}}})
    );
    assert!(!serde_json::to_string(&served).unwrap().contains("ciphertext-of"));
}
...
async fn check_success_on_2xx() {
    ...
    assert_eq!(seen.url, "https://dev123.service-now.com/api/now/table/sys_user?sysparm_limit=1");
...
async fn check_failure_on_non_2xx_carries_status_and_excerpt() {
    ...
    assert_eq!(outcome.message.as_deref(), Some("HTTP 500: upstream exploded"));
}
